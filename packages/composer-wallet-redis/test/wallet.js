...
describe('Composer wallet implementation', function () {
        ...
        CLOUD_CONFIG.correctConfigs.forEach((cfg) => {
            ...
            describe('Simple Path', () => {
                ...
                describe('#contains', async function () {
                    ...
                    it('should return true for present', async function () {
                        await wallet.put('IExist', 'I think therefore I\'ve got a headache');
                        return wallet.contains('IExist').should.eventually.be.true;
                    });

                    it('should return true for present - with full type', async function () {
                        await wallet.put('IExist', 'I think therefore I\'ve got a headache');
                        return wallet.contains('testing/IExist').should.eventually.be.true;
                    });
                });
    ...
    describe('#redis-only', ()=>{
        ...
        beforeEach(async () => {
            sandbox = sinon.sandbox.create();
            let config = CLOUD_CONFIG.correctConfigs[0];
            config.namePrefix = 'testing';
        ...
        it('should make sure invalid types on get are rejected', async function () {
            sandbox.stub(wallet,'_determineType').returns('weakness');
            sandbox.stub(wallet,'_encodeType').returns('being human');
            await wallet.put('Batman', 'Breathe in your fears. Face them. To conquer fear, you must become fear.');
            return wallet.get('Batman').should.eventually.be.rejectedWith(/Unkown type being stored weakness/);
        });

        // needs to be an independant test - as this is white box testing
        it('should ensure defensive checks are working', async function () {

            (()=>{
                wallet._encodeType('batman','villian');
            }).should.throw(/Unkown type being stored villian/);
